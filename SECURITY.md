...
UniversalOps is a 100% local desktop application with no telemetry, cloud dependencies, or network services (other than optional local LLM integration via Ollama).

If you discover a security vulnerability in UniversalOps, please report it privately by emailing the project maintainer (see `git log` for contact). **Do not** open a public GitHub issue for security vulnerabilities.
...
- UniversalOps runs with the privileges of the user who launches it.
- The PowerShell and Bash terminal features execute commands as the current user.
- Only run UniversalOps on systems where you trust the user account.
