...
async function wailsCall(method: string, ...args: any[]): Promise<any> {
  const go = getGo()
  if (!go) return null
...
export async function discoverLocalNetwork(): Promise<TopologyDevice[]> {
  try {
    ...
    return nodes.map((n: any) => ({
      id: n.id || `discovered-${Math.random().toString(36).slice(2, 8)}`,
      type: n.type || 'client',
      label: n.label || n.ip || 'Unknown',
      ip: n.ip || '',
      mac: n.mac || '',
...
export async function loadTopologyFromFile(): Promise<{ devices: TopologyDevice[]; connections: TopologyConnection[] } | null> {
  try {
    const result = await wailsCall('NetDesign.LoadTopology')
    if (!result) return null
    const devices = typeof result.nodes === 'string' ? JSON.parse(result.nodes) : (result.nodes || [])
    const connections = typeof result.edges === 'string' ? JSON.parse(result.edges) : (result.edges || [])
