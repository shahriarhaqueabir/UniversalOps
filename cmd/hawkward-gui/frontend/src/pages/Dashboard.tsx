      ...
      <div className="flex-1 min-w-0 w-full">
        ...
        <div className="flex gap-1 items-end h-12 w-full bg-panel-3 p-2 rounded-xl border border-border shadow-inner">
          {cpuHistoryBars.length > 0 ? cpuHistoryBars.map((val, i) => (
            <div
              key={i}
              className="flex-1 rounded-[2px] transition-all hover:scale-y-125 hover:opacity-80"
              style={{
...
export function Dashboard({ onNavigate }: { onNavigate?: (page: Page) => void }) {
  ...
  useEvents('metrics', useCallback((payload: any) => {
    ...
    if (!d || !d.cpu || !d.memory || !d.disk) {
      console.warn('[Dashboard] Incomplete metrics payload received, skipping update')
      return
    }
    setData(d)
    setLastUpdated(new Date())
    ...
    <div className="p-8 space-y-8 overflow-y-auto h-full scroll-smooth">
      ...
      <div className="flex items-center justify-between border-b border-[var(--color-border)] pb-6">
        ...
        <div className="flex gap-3">
          <button onClick={runQuickDiag} className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[var(--color-panel-2)] border border-[var(--color-border)] text-[var(--color-text)] text-sm font-semibold hover:bg-[var(--color-panel-3)] transition-all active:scale-95">
            <Zap size={16} className="text-[var(--color-warning)]" /> Quick Diagnostic
          </button>
          <button onClick={generateBriefing} className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[var(--color-accent)] text-white text-sm font-semibold hover:opacity-90 transition-all active:scale-95">
            <FileSearch size={16} /> Generate Briefing
          </button>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-xs font-semibold text-[var(--color-text-faint)] uppercase tracking-widest">Resource Compute Layer</h2>
          <div className="h-px flex-1 bg-[var(--color-border)]" />
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[var(--color-panel)] border border-[var(--color-border)] rounded-[var(--radius-lg)] p-6 shadow-lg flex flex-col">
          <h3 className="text-base font-bold text-[var(--color-text)] uppercase tracking-wider mb-4 flex items-center gap-2">
            <Activity size={18} className="text-[var(--color-accent)]" /> Compute Timeline
          </h3>
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          <div className="h-px flex-1 bg-[var(--color-border)]" />
          <h2 className="text-xs font-semibold text-[var(--color-text-faint)] uppercase tracking-widest">Functional Subsystems</h2>
          <div className="h-px flex-1 bg-[var(--color-border)]" />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          {opsItems.map((item) => (
            <button
              ...
              className="bg-[var(--color-panel-2)] border border-[var(--color-border)]/50 rounded-[var(--radius-lg)] p-5 transition-all duration-200 hover:bg-[var(--color-accent-soft)] hover:border-[var(--color-accent)]/30 group relative overflow-hidden text-left w-full card-hover"
            >
              <div className="absolute -bottom-4 -right-4 opacity-[0.03] group-hover:opacity-[0.08] transition-all group-hover:scale-150">
                {item.icon}
              </div>
              <div className="flex flex-col gap-3 relative z-10">
                <div className="w-9 h-9 rounded-lg bg-[var(--color-panel-3)] border border-[var(--color-border)] flex items-center justify-center text-[var(--color-accent)]">
                  {item.icon}
                </div>
                <div>
                  <h4 className="text-sm font-bold text-[var(--color-text)] mb-0.5">{item.label}</h4>
                  <p className="text-[10px] font-medium text-[var(--color-text-faint)] uppercase tracking-wider group-hover:text-[var(--color-accent)] transition-colors">Launch Module</p>
                </div>
              </div>
            </button>
          ))}
      ...
      {/* ── Quick Diagnostic Modal (Radix Dialog) ── */}
      <Dialog.Root open={diagOpen} onOpenChange={setDiagOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 bg-[var(--color-panel)] border border-[var(--color-border)] rounded-[20px] p-8 w-full max-w-2xl max-h-[80vh] overflow-y-auto shadow-2xl data-[state=open]:animate-in data-[state=open]:zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:zoom-out-95">
            <div className="flex items-center justify-between mb-6">
              <Dialog.Title className="text-2xl font-bold text-[var(--color-text)] flex items-center gap-3">
                <Zap size={24} className="text-[var(--color-warning)]" /> Quick Diagnostic Results
              </Dialog.Title>
            ...
            {diagLoading ? (
              <div className="flex items-center justify-center py-16">
                ...
                <span className="ml-4 text-lg font-bold text-[var(--color-text-faint)]">Running diagnostics...</span>
              </div>
            ) : (
              <div className="space-y-3">
                {diagResults.map((r, i) => (
                  <div key={i} className={`flex items-start gap-4 p-4 rounded-xl border ${diagColor(r.status)}`}>
                    <div className="mt-1 shrink-0">{diagIcon(r.status)}</div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-1">
                        <span className="text-sm font-bold text-[var(--color-text)] uppercase tracking-wider">{r.category}</span>
                        <span className="text-xs font-semibold text-[var(--color-text-faint)] tabular-nums">{r.value.toFixed(1)}{r.unit}</span>
                      </div>
                      <p className="text-sm text-[var(--color-text-dim)] leading-relaxed">{r.message}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {/* ── Briefing Modal (Radix Dialog) ── */}
      <Dialog.Root open={briefingOpen} onOpenChange={setBriefingOpen}>
        <Dialog.Portal>
          <Dialog.Overlay className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm data-[state=open]:animate-in data-[state=open]:fade-in data-[state=closed]:animate-out data-[state=closed]:fade-out" />
          <Dialog.Content className="fixed left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 bg-[var(--color-panel)] border border-[var(--color-border)] rounded-[20px] p-8 w-full max-w-3xl max-h-[80vh] overflow-y-auto shadow-2xl data-[state=open]:animate-in data-[state=open]:zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:zoom-out-95">
            <div className="flex items-center justify-between mb-6">
              <Dialog.Title className="text-2xl font-bold text-[var(--color-text)] flex items-center gap-3">
                <ScrollText size={24} className="text-[var(--color-accent)]" /> Operations Briefing
              </Dialog.Title>
              <Dialog.Close className="text-[var(--color-text-faint)] hover:text-[var(--color-text)] transition-colors rounded-lg p-1 hover:bg-[var(--color-sidebar-hover)]">
                <XCircle size={20} />
              </Dialog.Close>
            ...
            {briefingLoading ? (
              <div className="flex items-center justify-center py-16">
                <Loader2 size={32} className="text-[var(--color-accent)] animate-spin" />
                <span className="ml-4 text-lg font-bold text-[var(--color-text-faint)]">Synthesizing briefing...</span>
              </div>
            ) : briefingSections.length === 0 ? (
              <p className="text-[var(--color-text-faint)] text-center py-8">No briefing data available.</p>
            ) : (
              <div className="space-y-4">
                {briefingSections.map((s, i) => (
                  <div key={i} className="p-5 rounded-xl border border-[var(--color-border)] bg-[var(--color-panel-2)]">
                    <div className="flex items-center gap-3 mb-2">
                      {s.level === 'critical' ? <AlertTriangle size={18} className="text-[var(--color-danger)]" /> :
                        s.level === 'warning' ? <AlertCircle size={18} className="text-[var(--color-warning)]" /> :
                          <Info size={18} className="text-[var(--color-accent)]" />}
                      <h3 className="text-sm font-bold text-[var(--color-text)] uppercase tracking-wider">{s.title}</h3>
                    </div>
                    <p className="text-sm text-[var(--color-text-dim)] whitespace-pre-wrap leading-relaxed">{s.content}</p>
                  </div>
                ))}
              </div>
            )}
