...
const BAR_GREEN = 'var(--color-success)'
const BAR_AMBER = 'var(--color-warning)'
const BAR_RED = 'var(--color-danger)'
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
              ...
              <div className="lg:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-panel border border-border rounded-[var(--radius-lg)] p-8 shadow-2xl">
                  ...
                  <Bar label="Physical Allocation" value={memInfo.used_percent} color="var(--color-success)" showLabel={false} />
                ...
                {batteryInfo?.detected === true && (
                  <div className="bg-panel border border-border rounded-[var(--radius-lg)] p-8 shadow-2xl">
                    ...
                    <div className="space-y-4">
                      ...
                      <Bar label="Charge" value={batteryInfo.percent} color={pctColor(batteryInfo.percent)} showLabel={false} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div className="bg-panel border border-border rounded-[var(--radius-lg)] p-8 shadow-2xl">
                ...
                <div className="space-y-4">
                  {[...topProcesses].sort((a, b) => (b.read_bytes + b.write_bytes) - (a.read_bytes + a.write_bytes)).slice(0, 5).map(p => (
                    <div key={p.pid} className="flex items-center justify-between">
                      <div className="flex flex-col min-w-0">
                        <span className="text-sm font-medium text-text truncate">{p.name}</span>
                        <span className="text-xs font-bold text-text-faint">PID {p.pid}</span>
                      </div>
                      <span className="text-sm font-bold text-warning tabular-nums ml-4">
                        {((p.read_bytes + p.write_bytes) / 1024 / 1024).toFixed(1)} MB
                      </span>
                    </div>
                  ))}
                  {topProcesses.length === 0 && <p className="text-text-dim text-sm text-center py-4">No data</p>}
