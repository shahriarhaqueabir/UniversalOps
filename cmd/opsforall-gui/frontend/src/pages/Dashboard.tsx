      ...
      <div className="space-y-6 flex-1">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-2">
            {redFlags.map((flag) => (
              <div key={flag} className="flex items-start gap-3 bg-panel-2 p-4 rounded-2xl border border-border group/flag hover:border-danger/30 transition-colors">
    ...
    >
      ...
      <div className="flex-1 min-w-0 w-full">
        ...
        <div className="flex gap-1 items-end h-12 w-full bg-panel-3 p-2 rounded-xl border border-border shadow-inner">
          {healthTrend.length > 0 ? healthTrend.map((point) => (
            <div
              key={point.day}
      ...
      {insights.length > 0 && (
        <div className="space-y-1.5 border-t border-border/50 pt-3">
          ...
          {insights.slice(0, 2).map((insight) => (
            <div key={`${insight.title}-${insight.timestamp}`} className="flex items-start gap-2 bg-panel-2 rounded-lg p-2 border border-border/50">
              <div className={cn(
