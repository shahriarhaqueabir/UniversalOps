...
export function CommandOutput({ output, title }: CommandOutputProps) {
  return (
    <div className="bg-[var(--color-panel-3)] border border-[var(--color-border)] rounded-xl overflow-hidden">
      {title && (
        <div className="flex items-center gap-2 px-4 py-2 border-b border-[var(--color-border)] bg-[var(--color-panel-2)]">
      ...
      <pre className="p-4 text-sm font-mono text-[var(--color-text-dim)] overflow-x-auto whitespace-pre-wrap">
        {output || 'No output'}
      </pre>
    </div>
  )
